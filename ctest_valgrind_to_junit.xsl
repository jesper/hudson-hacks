...
<xsl:output method="xml" indent="yes"/>
    ...
    <xsl:template match="DynamicAnalysis/Test">
        ...
        <xsl:if test="Results/Defect/@type = 'Memory Leak'">
	    ...
	    <testcase name="{$testcasename}" classname="ValgrindTests" time="{$exectime}">

	    <xsl:variable name="failtype">
                <xsl:for-each select="Results/Defect">
	            <xsl:if test="@type = 'Memory Leak'">
		        <xsl:value-of select="."/>
                    </xsl:if>
                </xsl:for-each>
	    </xsl:variable>

            <error message="{$failtype} Memory Leaks"><xsl:value-of select="Log" /></error>
